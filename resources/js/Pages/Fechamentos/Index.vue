...
<template>
    ...
    <ErpLayout>
        <template #header-title>Gestão de Fechamentos</template>
        <template #header-subtitle>Controle de Competências e Integração Bsoft</template>
        
        <template #header-actions>
            <button @click="mostrarFormulario = !mostrarFormulario" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-2 px-4 rounded-xl shadow-[0_4px_12px_rgba(37,99,235,0.3)] transition-all flex items-center gap-2">
        ...
        <div class="space-y-6 max-w-7xl">
            ...
            <div v-if="mostrarFormulario" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 border-t-4 border-t-blue-500 animate-fade-in-down">
                ...
                <form @submit.prevent="criarFechamento" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    <div class="md:col-span-2">
                        ...
                        <input v-model="form.titulo" type="text" placeholder="Ex: Janeiro - 1ª Quinzena" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Data Inicial</label>
                        <input v-model="form.data_inicio" type="date" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Data Final</label>
                        <input v-model="form.data_fim" type="date" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    </div>
                    ...
                    <div class="md:col-span-4 flex justify-end gap-3 mt-2">
                        ...
                        <button type="submit" :disabled="form.processing" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-sm transition disabled:opacity-50">
            ...
            <div v-if="fechamentos.length > 0" class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div v-for="fechamento in fechamentos" :key="fechamento.id" class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all group flex flex-col">
                    <div class="p-6 flex-grow">
                        ...
                        <div class="bg-slate-50 rounded-lg p-3 border border-slate-100 mb-4">
                            ...
                            <p class="text-base font-bold text-blue-600">{{ formatarData(fechamento.data_vencimento) }}</p>
                        </div>
                    ...
                    <div class="bg-slate-50 border-t border-slate-200 p-4 flex flex-col gap-2">
                        ...
                        <Link :href="`/fechamentos/${fechamento.id}`" class="block w-full text-center bg-white border border-slate-300 hover:border-blue-500 text-slate-700 hover:text-blue-600 font-bold py-2 rounded-lg text-sm transition-colors">
            ...
            <div v-else class="bg-white rounded-xl border border-dashed border-slate-300 p-16 text-center max-w-3xl mx-auto">
