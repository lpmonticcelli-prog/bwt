...
// Carregando as funções reutilizáveis
            ...
            <table class="w-full text-left text-sm border-collapse whitespace-nowrap">
                <thead class="bg-slate-50/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-10">
                    <tr>
                        ...
                        <th class="py-4 px-5 font-bold text-slate-500 text-[10px] uppercase tracking-wider text-right">Custo E4LOG</th>
                        <th class="py-4 px-5 font-bold text-slate-500 text-[10px] uppercase tracking-wider text-right">Tabela Certa</th>
                        <th class="py-4 px-5 font-bold text-blue-600 text-[10px] uppercase tracking-wider text-right">Receita Sol Fácil</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="frete in fretes" :key="frete.id" @click="emit('abrir-dossie', frete.arquivo)" class="cursor-pointer hover:bg-emerald-50/30 transition-colors group">
                        ...
                        <td class="py-3.5 px-5 text-slate-700 font-semibold text-right">{{ formatMoney(frete.cobrado) }}</td>
                        <td class="py-3.5 px-5 text-emerald-600 font-semibold text-right">{{ formatMoney(frete.correto) }}</td>
                        <td class="py-3.5 px-5 text-blue-600 font-bold text-right">{{ formatMoney(obterReceitaAssociada(frete, viagens)) }}</td>
                        ...
                        <td class="py-3.5 px-5">
                            <span class="text-[10px] font-bold px-2 py-1 rounded shadow-sm inline-block bg-emerald-50 border border-emerald-100 text-emerald-600 max-w-[260px] truncate" :title="descobrirMotivo(frete)">
                    ...
                    <tr v-if="fretes.length === 0">
                        <td colspan="7" class="text-center py-10 text-slate-500 font-medium">
                            <div class="flex flex-col items-center justify-center space-y-2">
                                ...
                                <span>Não há custos aprovados no momento para este lote.</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
