...
<head>
    ...
    <style>
        ...
        .ref-table th.header-blue { background-color: #0284c7; color: white; text-align: center; font-size: 8px;}
...
<body>
    ...
    <div class="header">
        <h1>Relatório Executivo de Auditoria BWT (Sol Fácil)</h1>
        ...
        <table class="panel-table">
            <tr>
                <td style="width: 45%; border-right: 1px solid #cbd5e1;">
                    ...
                    <div><span class="font-bold">Faturamento Realizado:</span> <span class="val-total">R$ {{ number_format($resumo['total_cobrado'], 2, ',', '.') }}</span></div>
                    <div style="margin-bottom: 4px;"><span class="font-bold text-indigo">Faturamento Correto (SLA):</span> <span class="font-bold">R$ {{ number_format($resumo['total_sla'], 2, ',', '.') }}</span></div>
                    
                    <div><span class="font-bold text-red">Deixamos de Faturar (Perda):</span> R$ {{ number_format($resumo['faturado_a_menos'], 2, ',', '.') }}</div>
                    <div><span class="font-bold text-green">Faturado Indevidamente (A maior):</span> R$ {{ number_format($resumo['faturado_a_mais'], 2, ',', '.') }}</div>
                    
                    <div style="margin-top: 4px; padding-top: 4px; border-top: 1px dashed #cbd5e1;">
                        <span class="font-bold">Impacto Líquido:</span> 
                        <strong class="{{ $resumo['balanco_geral'] >= 0 ? 'text-green' : 'text-red' }}">
                            R$ {{ number_format(abs($resumo['balanco_geral']), 2, ',', '.') }}
                            {{ $resumo['balanco_geral'] >= 0 ? '(Lucro/A Maior)' : '(Prejuízo)' }}
                        </strong>
                    </div>
                </td>
                
                <td style="width: 55%; padding-left: 5px;">
                    <div class="font-bold text-indigo" style="font-size: 7.5px;">TABELA BWT X SOL FÁCIL</div>
                    <table class="ref-table">
                        <tr>
                            <th class="header-blue">SÃO PAULO</th>
                            <th colspan="3" style="background-color: white; border: none;"></th>
                        </tr>
                        <tr>
                            <th class="text-center">ESTADO DE SP<br>REGIÕES</th>
                            <th class="text-center">VALOR DE<br>FRETE MÍNIMO</th>
                            <th class="text-center">% DA NF</th>
                            <th class="text-center">TEMPO DE ENTREGA<br>APÓS A COLETA</th>
                        </tr>
                        <tr>
                            <td>REGIÃO 1 - REGIÃO DE SP E CAMPINAS</td>
                            <td class="text-center">R$ 350,00</td>
                            <td class="text-center">3,00</td>
                            <td class="text-center">ATÉ 3 DIAS ÚTEIS</td>
                        </tr>
                        <tr>
                            <td>REGIÃO 2 - CENTRO DO ESTADO</td>
                            <td class="text-center">R$ 350,00</td>
                            <td class="text-center">3,50</td>
                            <td class="text-center">ATÉ 3 DIAS ÚTEIS</td>
                        </tr>
                        <tr>
                            <td>REGIÃO 3 - REGIÕES DISTANTES</td>
                            <td class="text-center">R$ 550,00</td>
                            <td class="text-center">4,00</td>
                            <td class="text-center">ATÉ 5 DIAS ÚTEIS</td>
                        </tr>
                        <tr>
                            <td>REGIÃO 4 - BAIXA DEMANDA</td>
